...
namespace palaBenchmark
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            /*var ps = new PostStack(12384);

            foreach (PostRecord el in ps)
            {
                Console.WriteLine($"------------------------------------");
                Console.WriteLine($"ID : {el.ID}");
                Console.WriteLine($"UUID : {el.UUID}");
                Console.WriteLine($"LCT : {el.LCT}");
                Console.WriteLine($"Title : {el.Title}");
                Console.WriteLine($"Summary : {el.Summary}");
                Console.WriteLine($"Content : {el.Content}");
                Console.WriteLine($"Label : {el.Label}");
                Console.WriteLine($"Cover : {el.Cover}");
                Console.WriteLine($"ArchiveName : {el.ArchiveName}");
                Console.WriteLine($"CT : {el.CT}");
                Console.WriteLine($"Mode : {el.Mode}");
                Console.WriteLine($"Type : {el.Type}");
                Console.WriteLine($"User : {el.User}");
                Console.WriteLine($"UVCount : {el.UVCount}");
                Console.WriteLine($"StarCount : {el.StarCount}");
                Console.WriteLine($"ArchiveID : {el.ArchiveID}");
            }*/

            /*var el = new PostRecord("8bc10c6f84c04985bad8a99ee38202e4", false);
            Console.WriteLine($"------------------------------------");
            Console.WriteLine($"ID : {el.ID}");
            Console.WriteLine($"UUID : {el.UUID}");
            Console.WriteLine($"LCT : {el.LCT}");
            ...
            /*var pt = new PostTree(12384);
            var ptls = pt.Leaves;
            var ptns = pt.Nodes;

            foreach (PostStack item in ptls)
            {
                Console.WriteLine(item.ID);
            }
            Console.WriteLine("_________________________________________________________________________");
            foreach (PostStack item in ptns)
            {
                Console.WriteLine(item.ID);
            }*/
